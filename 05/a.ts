...
for (let i = 0; i < lines.length; i++) {
    ...
    if (i === 0) {
        const seedsLine = line.substring(line.indexOf(":") + 1, line.length);
        seeds.push(...seedsLine.split(" ").filter((s) => s !== "").map(x => parseInt(x)));
        continue;
    }
...
for (const seed of seeds) {
    ...
    seedToLocation.push({ seed, location });
}

const minLocation = Math.min(...seedToLocation.map(x => x.location));
